...
const normalizeLevel = (raw) => {
  ...
  // 关键：增加对 M / L / H 的识别！
  if (v === '1' || v === 'critical' || v === 'high' || v === 'h' || v === 'serious') return 'critical';
  if (v === '2' || v === 'warning' || v === 'medium' || v === 'm' || v === 'warn') return 'warning';
  return 'info';
};
...
return {
  alarm_rows: rows.map((row) => ({
    // 1. 级别：识别 M
    level: normalizeLevel(row?.alarm_level),
    // 2. 标题
    title: String(row?.alarm_config_name ?? row?.name ?? ''),
    // 3. 详情：取 content
    detail: String(row?.content ?? ''),
    // 4. 设备来源：从数组里取名字
    source: String(row?.alarm_device_list?.[0]?.name ?? ''),
    // 5. 时间：取 create_at
    time: fmtTime(row?.create_at),
  })),
  alarm_total: Number(payload?.total ?? rows.length ?? 0),
  latest_alarm_level: normalizeLevel(latest?.alarm_level),
  latest_alarm_title: String(latest?.alarm_config_name ?? latest?.name ?? ''),
};
`,
    },
